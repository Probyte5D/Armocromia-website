        ...
        col.innerHTML = `
          <div class="card border-0 shadow rounded-4 h-100">
            ...
            <div class="card-body text-center">
              ...
              <p class="card-text text-muted small">${p.description.substring(0,60)}...</p>
              <p class="text-primary fw-semibold">€${p.price}</p>
              <button class="btn btn-outline-dark rounded-pill px-4 apri-album" data-bs-toggle="modal" data-bs-target="#albumModal">Guarda Album</button>
            </div>
          </div>`;
        row.appendChild(col);
      });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  fetch('https://fakestoreapi.com/products?limit=6')
        ...
        col.innerHTML = `
          <div class="card border-0 shadow rounded-4 h-100">
            ...
            <div class="card-body text-center">
              ...
              <p class="card-text text-muted small">${p.description.substring(0, 60)}...</p>
              <p class="text-primary fw-semibold">€${p.price}</p>
              <button class="btn btn-outline-dark rounded-pill px-4 apri-album" data-bs-toggle="modal" data-bs-target="#albumModal" data-product-id="${p.id}">Guarda Album</button>
            </div>
          </div>`;
        row.appendChild(col);
      });
    ...
    .catch(err => {
      ...
      row.innerHTML = '<p class="text-danger text-center">Errore nel caricamento dei prodotti.</p>';
    });

  // Aggiungi l'evento per il bottone "Guarda Album"
  document.querySelectorAll('.apri-album').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      // Nuova API (per esempio: Unsplash API)
      const API_KEY = '1J5XZG6HEjef4TKrOiBtmHWoMJVdLRUIxplL9GJzFHBeI64zgRPny53m'; // Sostituisci con la tua chiave API di Unsplash
      const query = 'fashion'; // Modifica la query in base al prodotto

      fetch(`https://api.unsplash.com/photos/random?query=${encodeURIComponent(query)}&count=9`, {
      ...
      .then(data => {
        ...
        data.forEach(photo => {
          ...
          col.innerHTML = `
            <img src="${photo.urls.regular}" class="img-fluid rounded shadow-sm" alt="${photo.alt_description}" title="Ph: ${photo.user.name}">
          `;
          row.appendChild(col);
        });
